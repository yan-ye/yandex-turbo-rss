...
class TR {
    ...
    itemsProcessing(items, channel) {
        ...
        items.forEach(function (item) {
            ...
            self.pushIfConditionTrue(item.category, itemValues, {category: item.category});
    ...
    generateXML(options) {
        ...
        if (options.title) channel.push({title: {_cdata: options.title}});
        if (options.link) channel.push({link: options.link});
        if (options.description) channel.push({description: {_cdata: options.description}});
        channel.push({language: options.language});
        if (options.lastBuildDate) channel.push({lastBuildDate: options.lastBuildDate});
        if (options.pubDate) channel.push({pubDate: options.pubDate});
